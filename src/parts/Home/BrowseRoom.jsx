	...
	return dummy.map((item, index) => {
		return (
			<div
				...
				style={{
					height: index === 0 ? "180px" : "auto ",
				}}>
				<div className="bg-gray-300 rounded-lg w-full h-full">
					<div className={`overlay ${ratio?.meta?.[item.ratio.md]}`}>
						<div className="w-24 h-3 bg-gray-400 mt-3 rounded-full" />
						<div className="w-36 h-3 bg-gray-400 mt-2 rounded-full" />
					</div>
				</div>
			</div>
		)
...
const BrowseRoom = () => {
	const { data, isLoading, run } = useAsync({ data: { username: "" } });
	useEffect(() => {
